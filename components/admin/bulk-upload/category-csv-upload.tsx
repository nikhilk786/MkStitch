...
import {
  bulkUploadCategoriesAction,
  type BulkCategoryUploadItem,
  type BulkUploadResult,
} from "@/app/admin/actions";

const previewColumns = ["name", "slug", "isActive"] as const;

export function CategoryCsvUpload() {
  const [items, setItems] = useState<BulkCategoryUploadItem[]>([]);
  ...
  function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    Papa.parse<BulkCategoryUploadItem>(file, {
      header: true,
      skipEmptyLines: true,
      complete: (parseResult: ParseResult<BulkCategoryUploadItem>) => {
  ...
  function handleUpload() {
    startTransition(async () => {
      setResult(await bulkUploadCategoriesAction(items));
    });
  }
  ...
  return (
    <div className="boutique-shadow rounded-[2rem] border border-[#f0d9e8] bg-white p-6 sm:p-8">
      <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
        This sample CSV is configured for https://ik.imagekit.io/nicks.
      </div>

      <div className="mt-6">
        <label htmlFor="category-csv" className="text-sm font-semibold text-[#694E4E]">
          Upload category CSV
        </label>
        <input
          id="category-csv"
